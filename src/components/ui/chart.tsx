  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'grid min-w-32 items-start gap-1.5 rounded-sm border border-border-white-10 bg-card px-2.5 py-1.5 text-xs shadow-md',
          className
        )}
        {...props}
      >
      ...
      {payload.map((item) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-text-muted-60 text-xs">{itemConfig?.label || item.value}</span>
          </div>
        );
      })}
    </div>
