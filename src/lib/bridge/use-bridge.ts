            ...
            try {
                ...
                if (depositProtocol === 'hyperliquid' && depositFeePayer) {
                    try {
                        ...
                        const arbitrumBalance = await getUSDCBalanceOnArbitrum(
                            walletAddress as `0x${string}`
                        );
                        ...
                        if (!permitResult.success || !permitResult.typedData) {
                            throw new Error(
                                permitResult.error || 'Failed to create USDC permit'
                            );
                        }
                        ...
                        if (!signatureResult.success || !signatureResult.signature) {
                            throw new Error(
                                signatureResult.error || 'Failed to sign USDC permit'
                            );
                        }
