/**
 * Bridge Signing Utility
 * Handles EIP-712 signing for bridge permits using Turnkey
 */
...
/**
 * Sign EIP-712 permit data with Turnkey
 * @param permitData - EIP-712 formatted permit data
 ...
 */
export async function signPermitWithTurnkey(
  permitData: {
    sign: {
      domain: {
        name: string;
  ...
  try {
    ...
    const domain = {
      ...permitData.sign?.domain,
      chainId: permitData?.sign?.domain?.chainId || 8453,
    };
    ...
    console.error('Error signing permit with Turnkey:', error);
