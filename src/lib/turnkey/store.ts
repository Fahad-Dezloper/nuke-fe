...
export const isCreatingWalletAtom = atom(
    (get) => get(turnkeyStateAtom).isCreatingWallet
);
export const userWalletsAtom = atom((get) => get(turnkeyStateAtom).userWallets);
export const turnkeySubOrgIdAtom = atom(
    (get) => get(turnkeyStateAtom).turnkeySubOrgId
);
...
export const signTransactionAtom = atom(
    ...
    async (
        _get,
        _set,
        { unsignedTx, walletId }: { unsignedTx: string; walletId: string }
    ) => {
        return await turnkeyClient.signTransaction(unsignedTx, walletId);
    }
);

export const createWalletAtom = atom(
    null,
