...
export const isTradingLoadingAtom = atom(
    (get) => get(tradingStateAtom).isLoading
);
export const tradingErrorAtom = atom((get) => get(tradingStateAtom).error);
export const lastTradingActionAtom = atom(
    (get) => get(tradingStateAtom).lastAction
);
export const isSigningInProgressAtom = atom(
    (get) => get(tradingStateAtom).signingInProgress
);
