...
export class HyperLiquidService {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw createError(
                    ...
                    new Error(`HyperLiquid API error: ${response.status} ${response.statusText} - ${errorText}`)
                );
            }
    ...
    async createPosition(
        ...
        try {
            ...
            let typedDataResult;
            try {
                typedDataResult = await this.perpOrders.createPerpPositionTypedData(
                    perpOrderRequest
                );
            } catch (error) {
                throw createError(
            ...
            if (hyperLiquidResponse.status !== 'ok') {
                const errorMsg =
                    ...
                    'Failed to create position (unknown error)';
    ...
    async closePosition(
        ...
        try {
            ...
            let typedDataResult;
            try {
                typedDataResult = await this.perpOrders.closePerpPositionTypedData(
                    perpOrderRequest
                );
            } catch (error) {
                throw createError(
            ...
            if (hyperLiquidResponse.status !== 'ok') {
                const errorMsg =
                    hyperLiquidResponse.response ||
                    'Failed to close position (unknown error)';
     ...
     */
    async fetchUserLeverage(
        userAddress: string,
        coin: string
    ): Promise<UserLeverageResponse> {
        try {
            if (!userAddress) {
                throw createError(ErrorCode.WALLET_ADDRESS_REQUIRED);
            }
