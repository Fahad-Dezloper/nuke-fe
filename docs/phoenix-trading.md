...
- **Invite path**: `NEXT_PUBLIC_PHOENIX_INVITE_CODE` **or** `NEXT_PUBLIC_PHOENIX_REFERRAL_CODE` (HTTP `activateInvite` / `activateInviteWithReferral`) before `registerTrader` succeeds
- **Flight (optional)**: `NEXT_PUBLIC_PHOENIX_FLIGHT_ENABLED=true` plus `NEXT_PUBLIC_PHOENIX_FLIGHT_BUILDER_AUTHORITY` and PDA/subaccount indices from the Phoenix Builder Dashboard
...
- `DEPOSIT_TO_PHOENIX` — same `DepositActionParams` shape as other deposit actions; deposits `amount_usd` (6dp USDC) from the user’s Solana USDC balance via Rise `buildDepositIxs`, Turnkey-signed.
